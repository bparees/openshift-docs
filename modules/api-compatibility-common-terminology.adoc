...
The AOE does not include components which may vary by installation such as Container Network Interface(CNI) plugin selection or extensions to the product such as admission hooks.  Components which integrate with the cluster at a level below the container environment may be subjected to additional variation between versions.
